/*******************************************************************************
 * Copyright (c) 2008, 2012 Jay Rosenthal, AGETO Service GmbH and others.
 *
...
public class X500PrincipalHelper {
	...
	public X500PrincipalHelper(final X500Principal principal) {
		parseDN(principal.getName(X500Principal.RFC2253));
	}

	private String findPart(final String attributeID) {
		return findSignificantPart(attributeID, MOSTSIGNIFICANT);
	}

	private String findSignificantPart(final String attributeID, final int significance) {
		...
		for (final Iterator iter = rdnNameArray.iterator(); iter.hasNext();) {
			final ArrayList nameList = (ArrayList) iter.next();
			final String namePart = (String) nameList.get(0);
	 ...
	 *            - X500Principal
	 */
	public void setPrincipal(final X500Principal principal) {
		parseDN(principal.getName(X500Principal.RFC2253));
	}
